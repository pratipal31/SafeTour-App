...
router.post('/', async (req, res) => {
  const { contacts, message, userName, location, latitude, longitude, action } = req.body;
  ...
  try {
    const results = [];
    // Simplified message to avoid Twilio trial account fraud detection (Error 30044)
    let emergencyMessage = message || `SafeTour Alert: ${userName || 'User'} needs help. Location: ${location || 'Unknown'}. Time: ${new Date().toLocaleString()}.`;
    
    // Add coordinates if available
    if (latitude && longitude) {
      emergencyMessage += ` GPS: ${latitude}, ${longitude}. Map: https://maps.google.com/?q=${latitude},${longitude}`;
    }
    
    emergencyMessage += ' Please contact them.';
      ...
      try {
        ...
        if (action === 'call' || action === 'both') {
          try {
            ...
            // Build call message with coordinates if available
            let callMessage = `Emergency Alert! ${userName || 'A SafeTour user'} needs immediate help. They are located at ${location || 'an unknown location'}.`;
            if (latitude && longitude) {
              callMessage += ` GPS coordinates: ${latitude}, ${longitude}.`;
            }
            callMessage += ' Please check on them immediately or call them back.';
            ...
            callResult = { success: true, sid: call.sid };
          } catch (callError) {
            console.error(`Call failed for ${contact.name}:`, callError);
        ...
        if (callResult) contactResult.call = callResult;
        if (smsResult) contactResult.sms = smsResult;
...
router.post('/sms', async (req, res) => {
  ...
  try {
    ...
    // Simplified message to avoid Twilio trial account fraud detection (Error 30044)
    const emergencyMessage = message || `SafeTour Alert: ${userName || 'User'} needs help. Location: ${location || 'Unknown'}. Time: ${new Date().toLocaleString()}. Please contact them.`;
